...
def on_message(mqttc, userdata, msg):
    ...
    if mensaje == 'El ganador es: player1' or mensaje == 'El ganador es: player2':
        print(mensaje)
        pygame.quit()                #Si uno de los diccionarios de barcos está vacio acabamos la partida
    elif mensaje == 'player2, Dame una celda: ':
        pass                         #Es un mensaje para el otro jugador
    elif len(mensaje) == 17:         #Es una celda repetida
        p, repetido = mensaje.split(',')
        if p == 'player1':
            print('Ya habías elegido esa celda')
            t[1] = True              #Ejecutamos el bucle del final
    elif mensaje == 'Empezamos, dame una celda: ' or mensaje == 'player1, Dame una celda: ':
        t[1] = True                  #Ejecutamos el bucle del final
        ...
        t[1] = True                  #Ejecutamos el bucle del final
        pygame.init()
        tablerop2[fil1][col1] = color #Cambiamos el color de la celda
        ...
        for filas in range(tablero_1):
            for columnas in range(tablero_2):
                color_celda = tablerop2[filas][columnas]
...
tablerop2 = p_board(tablero_1, tablero_2)
t = [tablerop2,False]
...
for c in barcos:
    ...
    if n < 6:
        ...
        barco = 'b5'
    n += 1
    mensaje = 'player1,' + barco + ',' + str(a) + ',' + str(b)
